...
TracksZdf = pd.DataFrame(RemoveTracksZ, columns = ['BadZ'], dtype=float)
new_combined_data_aggragated=new_combined_data.groupby(['Track_ID'])['z'].min().reset_index()
new_combined_data_aggregated=new_combined_data_aggregated.rename(columns={'z': "Bad_z"})
new_combined_data=pd.merge(new_combined_data, new_combined_data_aggragated, how="left", on=["Track_ID"])
